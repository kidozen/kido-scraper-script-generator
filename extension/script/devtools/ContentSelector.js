...
ContentSelector.prototype = {
	...
	initCssSelector: function(allowMultipleSelectors) {
		this.cssSelector = new CssSelector({
			...
			ignoredClasses: [
				"-kido-scraper-select-item-selected",
				"-kido-scraper-select-item-hover",
				"-kido-scraper-parent",
	...
	bindElementHighlight: function () {
		...
		$(this.$allElements).bind("mouseover.elementSelector", function(e) {
			...
			$(element).addClass("-kido-scraper-select-item-hover");
			...
			$(element).removeClass("-kido-scraper-select-item-hover");
		}.bind(this));
	},
	...
	highlightSelectedElements: function () {
		try {
			...
			$(ElementQuery(resultCssSelector, this.parent)).addClass('-kido-scraper-select-item-selected');
		}
		catch(err) {
	...
	highlightParent: function () {
		...
		if(!$(this.parent).is("body") && !$(this.parent).is("#webpage")) {
			$(this.parent).addClass("-kido-scraper-parent");
		}
	},
